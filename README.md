# 🏊 Piscine 42 Lisboa

Repositório dedicado aos projetos, exercícios e aprendizados desenvolvidos durante a experiência na Piscine da 42 Lisboa, considerada a melhor escola de programação do mundo.

## 🚀 Sobre a experiência

A Piscine é o processo intensivo de seleção e aprendizagem da escola 42. Ela é focada em desenvolvimento de software, resolução de problemas e aprendizado colaborativo.

Durante esse período, trabalhei em desafios práticos de programação com foco em:

- lógica computacional  
- algoritmos  
- resolução de problemas  
- pensamento estruturado  
- desenvolvimento em linguagem C  
- trabalho em equipe e peer learning  

---

## 🧠 Principais aprendizados

- Pensamento computacional aplicado  
- Estruturação lógica de soluções  
- Resolução de problemas complexos sob pressão  
- Autonomia no aprendizado técnico  
- Colaboração e revisão entre pares  
- Debugging e análise de erros  

---

## 🛠️ Tecnologias e temas

- Linguagem C  
- Shell  
- Git/GitHub  
- Algoritmos  
- Estruturas de dados  
- Terminal/Linux  

---

## 👨‍💻 Sobre mim

Sou Product Manager com foco em produtos digitais, UX Research e estratégia de produto. A experiência na 42 Lisboa ampliou minha fluência técnica e capacidade de comunicação com times de engenharia, além de fortalecer habilidades analíticas e resolução de problemas.

---

## 🔗 Projetos relacionados

### 💼 Portfólio
👉 https://diolenho.github.io/portfolio-diogo/

### 🏳️‍⚧️ Guia Trans
👉 https://diolenho.github.io/guia-trans-app/#/

---

## 📚 Sobre a 42

A 42 é uma rede internacional de escolas de programação conhecida pelo modelo peer-to-peer e metodologia baseada em projetos práticos. 

---

## 📬 Contato

- 💼 LinkedIn: https://www.linkedin.com/in/diogopalmeida/  
- 🌐 Portfólio: https://chipped-leotard-652.notion.site/Diogo-Almeida-Product-Manager-31d2da75adeb80368a1aecb43415ad59  
